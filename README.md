...
It's been a long standing problem that access violations like a bad address for CopyMemory and other exceptions can't be handled by [tt]On Error[/tt]. One solution to that is Vectored Exception Handling (VEH). You can register a procedure to handle true exceptions like access violations, then set it to skip the offending instruction. 
...
This works by modifying the [tt]CONTEXT[/tt] structure, which contains among other things the contents of all registers, including the instruction pointer register- [tt]Eip[/tt] for 32bit, [tt]Rip[/tt] for 64bit. If an access violation is encountered, we skip the instruction by adding the instruction size-- this is where it gets the most complicated, and to be honest I used Claude AI for the functions to calculate the length, and don't totally understand it, since it's dynamic at runtime and not just looking at the disassembly on disk.

[tt]CONTEXT[/tt] and many other definitions in this project are entirely different for 32bit and 64bit, so the requirements are as follows:
